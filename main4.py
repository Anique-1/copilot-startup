...
async def fetch_city_industry_growth(city: str, industry: str) -> List[Dict]:
    params = {
        "engine": "google",
...
page = st.sidebar.radio("Select a page:", ["Startup Copilot", "City Startup Ideas", "City Startup Insights"])
        ...
        if not city or not industry:
            ...
            with st.spinner(f"Analyzing {industry} startups in {city}..."):
                try:
                    loop = asyncio.new_event_loop()
                    asyncio.set_event_loop(loop)
                    analysis = loop.run_until_complete(fetch_city_industry_growth(city, industry))
                except Exception as e:
